...
1. Ochrona tras i redirect po 401 do `/auth/login?redirectTo=<current>`; po logowaniu redirect do `/`.
2. Generator w układzie split view (desktop), z przełączeniem na zakładki na mobile; przycisk „Generuj” blokowany podczas requestu; licznik znaków 1000–10000.
3. Edycja propozycji inline z automatycznym „Edited”; kontrolki „Zaznacz/odznacz wszystkie” i skróty; walidacja długości pól.
4. Masowa akceptacja jednym POST; błędy per‑pozycja inline + toast; pełny rollback UI w razie błędu.
5. Brak trwałości efemerycznych propozycji; utrata po odświeżeniu akceptowalna.
6. Lista fiszek oparta o parametry w URL; paginacja, filtry, sort; skeletony i stany pustej listy.
7. Edycja/DELETE inline z dialogiem potwierdzenia.
8. Warstwa `apiClient` na `fetch` z obsługą 401/422/429 i `AbortController`.
9. Dostępność: pełna obsługa klawiatury, focus ringi, `aria-live` dla toastów, kontrast WCAG AA.
10. Bezpieczeństwo: przygotowanie pod JWT i strażnik tras; brak przechowywania wrażliwych danych w kliencie.
</matched_recommendations>
