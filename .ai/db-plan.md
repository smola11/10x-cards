...
CREATE TABLE IF NOT EXISTS public.flashcards (
  ...
  -- Spójność: manual => generation_id IS NULL; ai-full/ai-edited => generation_id IS NOT NULL
...
-- Triggery aktualizujące updated_at na UPDATE
...
- public.flashcards
  - id: bigserial PK
  - user_id: uuid NOT NULL → FK do `auth.users(id)` ON DELETE CASCADE
  - front: text NOT NULL, CHECK długość 1–200 znaków
  - back: text NOT NULL, CHECK długość 1–500 znaków
  - origin: enum `flashcard_origin` NOT NULL (brak wartości domyślnej; wartości: 'manual','ai-full','ai-edited')
  ...
  - CHECK spójności origin ↔ generation_id: manual ⇒ NULL; ai-full/ai-edited ⇒ NOT NULL
